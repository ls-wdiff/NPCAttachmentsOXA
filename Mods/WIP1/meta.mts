...
type ConditionType = `EQuestConditionType::${ConditionSubType}`;
...
type StructType = GetStructType<{
  ...
  Launchers: {
    "0": {
      ...
      Connections: {
        "0": {
          SID: "RSQ06_C00___SIDOROVICH_Start";
          Name: "";
        };
      };
    };
...
export const meta: Meta<StructType> = {
  ...
  interestingIds: [],
  onFinis1h(structs: StructType[], { file }) {
    ...
    const renderChildren = (v: StructType) => {
      return v["children"]
        .map(([id, pin]: [StructType, string]) => {
          ...
          if (pin) res += "{";
          if (
            0 /*id.entries.NodeType === "EQuestNodeType::Condition" || id.entries.NodeType === "EQuestNodeType::If"*/
          ) {
